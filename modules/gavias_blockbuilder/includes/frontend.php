	...
	if( is_array( $gbb_els ) ){
		...
		foreach( $gbb_els as $row ){
			if(isset($row['attr'])){
				...
				//Responsive
				if(isset($row_attr['hidden_lg']) && $row_attr['hidden_lg']){
					$row_attr[] = 'hidden-lg'; 
				}
				if(isset($row_attr['hidden_md']) && $row_attr['hidden_md']){
					$row_attr[] = 'hidden-md'; 
				}
				if(isset($row_attr['hidden_sm']) && $row_attr['hidden_sm']){
					$row_attr[] = 'hidden-sm'; 
				}
				if(isset($row_attr['hidden_xs']) && $row_attr['hidden_xs']){
					$row_attr[] = 'hidden-xs'; 
				}
				...
				if( $row_attr['bg_image'] ){
					$array_style[] 	= 'background-image:url('. $base_url . '/' . $row_attr['bg_image'] .')';
